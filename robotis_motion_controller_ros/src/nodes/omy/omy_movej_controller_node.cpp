...
#include "robotis_motion_controller_ros/nodes/omy/omy_movej_controller_node.hpp"
  ...
  try {
    ...
    kinematics_solver_ =
      std::make_shared<robotis_motion_controller::kinematics::KinematicsSolver>(urdf_path_,
        srdf_path_);
    qp_controller_ =
      std::make_shared<robotis_motion_controller::controllers::OpenManipulatorMoveJController>(
...
void OmyMoveJControllerNode::controlLoopCallback()
{
  ...
  try {
    ...
    if (movej_trajectory_active_ && elapsed < active_motion_duration_) {
      q_ref = robotis_motion_controller::common::math_utils::cubicVector(
                    elapsed, 0.0, active_motion_duration_, movej_start_, movej_goal_, zeros, zeros);
      qdot_ref = robotis_motion_controller::common::math_utils::cubicDotVector(
                    elapsed, 0.0, active_motion_duration_, movej_start_, movej_goal_, zeros, zeros);
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<robotis_motion_controller_ros::OmyMoveJControllerNode>();
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}
