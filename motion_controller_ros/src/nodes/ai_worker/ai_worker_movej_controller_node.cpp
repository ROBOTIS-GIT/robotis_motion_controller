...
#include "motion_controller_ros/nodes/ai_worker/ai_worker_movej_controller_node.hpp"
  ...
  left_active_motion_duration_(0.0)
{
  ...
  try {
    ...
    qp_filter_ = std::make_shared<motion_controller::controllers::AIWorkerMoveJController>(
  ...
  try {
    ...
    if (right_movej_target_initialized_) {
      ...
      if (right_movej_trajectory_active_ && elapsed < right_active_motion_duration_) {
        ...
        right_ref = motion_controller::common::math_utils::cubicVector(
          elapsed, 0.0, right_active_motion_duration_,
          right_movej_start_, right_movej_goal_, zeros, zeros);
        right_qdot = motion_controller::common::math_utils::cubicDotVector(
          elapsed, 0.0, right_active_motion_duration_,
          right_movej_start_, right_movej_goal_, zeros, zeros);
    ...
    if (left_movej_target_initialized_) {
      ...
      if (left_movej_trajectory_active_ && elapsed < left_active_motion_duration_) {
        const Eigen::VectorXd zeros = Eigen::VectorXd::Zero(q_feedback.size());
        left_ref = motion_controller::common::math_utils::cubicVector(
          elapsed, 0.0, left_active_motion_duration_,
          left_movej_start_, left_movej_goal_, zeros, zeros);
        left_qdot = motion_controller::common::math_utils::cubicDotVector(
          elapsed, 0.0, left_active_motion_duration_,
          left_movej_start_, left_movej_goal_, zeros, zeros);
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<motion_controller_ros::AIWorkerMoveJController>();
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}
